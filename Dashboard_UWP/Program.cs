    ...
    static class Program
    {
        ...
        public static void ModbusTcpMasterReadRegisters_SMA_MPPTs()
        {
            ...
            for (; ; )
            {
                foreach (IpInformation ipInfo in IP_Information)
                {
                    using (TcpClient client = new TcpClient())
                    {
                        try
                        {
                            ...
                            IAsyncResult ar = client.ConnectAsync(ipInfo.IP_Address, ipInfo.IP_Port);
                            WaitHandle wh = ar.AsyncWaitHandle;
                            try
                            {
                                if (!ar.AsyncWaitHandle.WaitOne(TimeSpan.FromSeconds(ipTimeout)))
                                {
                                    client.Dispose();
                            ...
                            Debug.WriteLine(timeNow.ToString("d"));

                            try
                            {
                                if (timeNow.ToString("d") == "00:00 AM")
                                {
                                    Task.Delay(60000);
                                }
                            }
                ...
                Task.Delay(300000);       //5 minutes sleep     
            }
        }
        //============================================== END ===========================================
